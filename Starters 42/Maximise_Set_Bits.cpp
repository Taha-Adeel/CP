#include <bits/stdc++.h>

using namespace std;

#define FAST 		ios::sync_with_stdio(0); cin.tie(0)
#define FOR(a,c) 	for (int a=0; a<c; ++a)
template<class T> struct v: vector<T>{using vector<T>::vector;
	void sort_asc() {sort(this->begin(), this->end());}
	void sort_dsc() {sort(this->begin(), this->end(), greater<T>());}
	friend ostream& operator << (ostream &outFunc, const v<T> &_v) {for(auto& i: _v) outFunc << i << ' '; return outFunc;}
	friend istream& operator >> (istream &inFunc, v<T> &_v) {for(auto& i: _v) inFunc >> i; return inFunc;}
};

#define f 	first
#define s 	second
constexpr char nl = '\n';
constexpr int MOD = 1000000007;
using ll   = long long int;
using p_ii = pair<int,int>;
using p_ll = pair<ll,ll>;
using v_i  = v<int>;
using v_ll  = v<ll>;

/*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*.*/

void solve(){
	ll N, K;
	cin >> N >> K;

	ll curr_sum = 0;
	ll pow_of_two = 0;
	ll answer = 0;
	v_ll count(32, 0);

	while(curr_sum < K){
		count[pow_of_two] += N;
		curr_sum += count[pow_of_two] * (1<<pow_of_two);
		pow_of_two++;
	}
	while(curr_sum != K){
		while(pow_of_two >= 0 && count[--pow_of_two] == 0)
			;

		ll num_of_subtractions = (curr_sum - K)/(1 << pow_of_two);
		curr_sum -= num_of_subtractions * (1 << pow_of_two);
		count[pow_of_two] -= num_of_subtractions;
	}
	for(auto n: count)
		answer += n;

	cout << answer << nl;
}

int main(){
	int T;
	cin >> T;
	while(T--){
		solve();
	}
	
	return 0;
}