...
void solve() {
    ...
    Z<MOD> term = 0;
    for (int i = k; i < n; i++) {
        // term += Z<MOD>::nCr(i, 0) * Z(m - k).pow(0) * Z(k - 1).pow(i) * Z(m).pow(n - i - 1);
        term += Z(k - 1).pow(i) * Z(m).pow(n - i - 1);
    }
    ans += term;

    for (int r = 1; r < k; r++) {
        term = (Z(m) * Z<MOD>::nCr(k, r) * Z(m).pow(n - k - 1) + Z(k - 1) * term - Z<MOD>::nCr(n, r) * Z(k - 1).pow(n - k)) / Z(m - k + 1);
        ans += term * Z(m - k).pow(r) * Z(k - 1).pow(k - r);
    }
