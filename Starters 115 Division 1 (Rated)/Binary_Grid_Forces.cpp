...
template<int MOD, int MxN> vector<Z<MOD, MxN>> Z<MOD, MxN>::fact;
...
void solve() {
    ...
    string s_cur = s[n - 1];
    for(int i = n - 2; i > 0; i--) {
        int cnt_11 = 0, cnt_10 = 0;
        FOR(j, m) {
            if(s[i][j] == '1' && s_cur[j] == '1') cnt_11++;
            if(s[i][j] == '1' && s_cur[j] == '0') cnt_10++;
        }

        string s_new = s[i];
        if(!(cnt_11 == 0 || (cnt_11 == 1 && cnt_10 == 0))) {
            FOR(j, m) {
                if(s_cur[j] == '1') s_new[j] = '1';
            }
        }
    ...
    int cnt_01 = 0, cnt_11 = 0;
    FOR(j, m) {
        if(s[0][j] == '0' && s_cur[j] == '1') cnt_01++;
        if(s[0][j] == '1' && s_cur[j] == '1') cnt_11++;
    }
    
    cout << Z<MOD, 1'000'001>::nCr(cnt_01 + cnt_11, cnt_01);
}
