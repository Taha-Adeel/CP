...
void solve(){
	...
	// vi a(n);
	// cin >> a;
	float ans = (14*13)/2.0 + (8+n)*(7+n)/2.0 + (6+n)*(5+n)/2.0;
    // int cur_pos = 0, cur_max_pos = a[0], next_max_pos = 0, next_i = 0;
    
    // // for(int i = 0; i < n; i++)
    // //     cout << a[i] << " ";
    
    // while(cur_max_pos < n-1){
    //     for(int i = cur_pos+1; i <= min(cur_max_pos, n-1); i++){
    //         if(i + a[i] >= next_max_pos){
    //             next_max_pos = i + a[i];
    //             next_i = i;
    //         }
    //     }
