...
void solve(){
	...
	vi a(n);
	cin >> a;
	map<int, V<pii>> friends;
	FOR(i, m){
		int x, y;
		cin >> x >> y;

		friends[x].pb({a[y-1], y});
		friends[y].pb({a[x-1], x});
	}
	
	for(auto& frds: friends)
		frds.S.sort();
	...
	if(m%2 == 0)
		...
		FOR(i, n){
			if(friends[aa[i].S].size() % 2 == 1)
				mn = min(mn, aa[i].F);
			else{
				for(auto f: friends[aa[i].S]){
					if(f.F > aa[i].F)
						break;
					if(friends[f.S].size()%2 == 0)
						mn = min(mn, aa[i].F + f.F);
					else
						break;
				}
			}
