...
void solve(){
	...
	SegmentTree<int> st_gcd(s, [](int a, int b){return __gcd(a, b);}, 0);
	SegmentTree<int> st_min(s, [](int a, int b){return min(a, b);}, INT_MAX);

	int q; cin >> q;
	while(q--){
		int l, r; cin >> l >> r; --l, --r;

		int gcd = st_gcd.query(l, r), mn = st_min.query(l, r);
		if(gcd == mn){
			int ans = 0;
			for(int i = l; i <= r; ++i)
				if(s[i] == mn)
