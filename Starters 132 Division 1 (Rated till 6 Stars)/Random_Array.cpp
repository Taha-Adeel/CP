...
void solve() {
    ...
    V<Z<MOD>> sum(n, 0), count(n, 0);
    V<Z<MOD>> dp(all_a.size(), 0);

    Z<MOD> cur_sum = 0, cur_elem_sum = 0, cur_elem = 0, cur_cnt = 0, elems_after = 0;
    for (int i = all_a.size() - 1; i >= 0; i--) {
        auto [x, idx] = all_a[i];
        if (Z(x) != cur_elem) {
            cur_sum += cur_elem_sum;
            elems_after += cur_cnt;
            cur_elem = x;
            cur_cnt = 0;
            cur_elem_sum = 0;
        ...
        dp[i] = Z(x);
        if (elems_after - count[idx] != Z(0)) {
            dp[i] += (cur_sum - sum[idx]) / (elems_after - count[idx]);
        }
        sum[idx] += dp[i];
        cur_elem_sum += dp[i];
        cur_cnt += 1;
