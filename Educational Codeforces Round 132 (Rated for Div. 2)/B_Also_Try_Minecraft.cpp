...
void solve(){
	...
	vll forward(n+1, 0), backward(n+1, 0);
	FOR(i, n-1)
		forward[i+1] = forward[i] + max(a[i] - a[i+1], 0);
	
	FOR_REV(i, n)
		backward[i] = backward[i + 1] + max(a[i] - a[i-1], 0);
	...
	FOR(i, m){
		...
		ll ans = 0;
		if(s <= t){
			ans = forward[t] - forward[s];
		}
		else{
			ans = backward[t+1] - backward[s+1];
		}
