...
template<class T> struct V: vector<T>{using vector<T>::vector;
	...
	map<T, int> freqs() {map<T, int> freqs; for(auto& i: *this) freqs[i]++; return freqs;}
...
void solve(){
	...
	int curr_l = 0, curr_r = freqs[')'];
	int max_dense = 2 * min(curr_l, curr_r);
	FOR(i, N){
		if(s[i] == '(')
			curr_l++;
		else
			curr_r--;
		max_dense = max(max_dense, 2 * min(curr_l, curr_r));
	}
	cout << N - max_dense;
}
