...
class SegmentTree{
	...
	T updateUtil(int cur_node, int i, T new_val)
	{
		if(tree[cur_node].l == tree[cur_node].r && tree[cur_node].l == i)
			tree[cur_node].val = new_val;
		else if(i >= tree[cur_node].l && i <= tree[cur_node].r)
			tree[cur_node].val = func(updateUtil(left_child(cur_node), i, new_val), updateUtil(right_child(cur_node), i, new_val));
...
public:
	...
	void update(T new_val, int i){
		arr[i] = new_val;
		updateUtil(0, i, new_val);
	}
};
