...
class LazySegmentAddUpdate: public LazySegmentTreeBase<ll, pll> {
    ll f_query(ll x, ll y) {return x + y;}
    pll& f_combine_tag(pll& tag, pll new_update) {
        if(new_update.S != 0) tag.S = new_update.S, tag.F = 0;
        if(new_update.F != 0) tag.F += new_update.F;
        return tag;
    }
    void consume_tag(int node_id) {
        if(tree[node_id].tag.S != 0) tree[node_id].val = tree[node_id].tag.S * (tree[node_id].r - tree[node_id].l + 1);
        tree[node_id].val += tree[node_id].tag.F * (tree[node_id].r - tree[node_id].l + 1);
    }

public:
    LazySegmentAddUpdate(V<ll>& a): LazySegmentTreeBase<ll, pll>(a) {
        identity_q = 0;
        identity_u = {0, 0};
        build(0, 0, n-1);
    }
