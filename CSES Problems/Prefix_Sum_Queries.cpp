...
class SegmentTree{
	...
	T constructSTUtil(int cur_node, int l, int r)
	{
		...
		if (tree[cur_node].l == tree[cur_node].r){
			...
			tree[cur_node].max_prefix_sum = max(arr[tree[cur_node].l], 0LL);
		}
		else{
			int m = (tree[cur_node].l + tree[cur_node].r)/2;
			tree[cur_node].sum = func(constructSTUtil(left_child(cur_node), l, m), constructSTUtil(right_child(cur_node), m + 1, r));
			tree[cur_node].max_prefix_sum = max(tree[left_child(cur_node)].max_prefix_sum, tree[left_child(cur_node)].sum + tree[right_child(cur_node)].max_prefix_sum);
		}
	...
	T updateUtil(int cur_node, int i)
	{
		if(tree[cur_node].l == tree[cur_node].r && tree[cur_node].l == i)
			...
			tree[cur_node].sum = func(updateUtil(left_child(cur_node), i), updateUtil(right_child(cur_node), i)),
			tree[cur_node].max_prefix_sum = max(tree[left_child(cur_node)].max_prefix_sum, tree[left_child(cur_node)].sum + tree[right_child(cur_node)].max_prefix_sum);
	...
	pair<T, T> queryUtil(int cur_node, int l, int r)
	{
		if(tree[cur_node].l >= l && tree[cur_node].r <= r) 
			return {tree[cur_node].sum, tree[cur_node].max_prefix_sum};
		else if(tree[cur_node].r < l || tree[cur_node].l > r)
			return {IDENTITY_VAL, -(1e17)};
		else{
			auto lval = queryUtil(left_child(cur_node), l, r), rval = queryUtil(right_child(cur_node), l, r);
			return {func(lval.F, rval.F), max(lval.S, lval.F + rval.S)};
		}
	}
