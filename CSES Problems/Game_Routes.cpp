...
void solve(){
	...
	bool found_a = false;
	V<int> predecessor(n, -1);
	for(auto& i: get_topological_ordering(graph)){
		if(i == 0) {found_a = true; graph[i].total_paths = 1; continue;}
		if(!found_a) continue;
		for(auto& e: graph[i].in_adj_list){
			graph[i].total_paths += graph[e].total_paths;
			graph[i].total_paths %= MOD;
		}
		if(i == n-1) break;
	}

	cout << graph[n-1].total_paths;
}
