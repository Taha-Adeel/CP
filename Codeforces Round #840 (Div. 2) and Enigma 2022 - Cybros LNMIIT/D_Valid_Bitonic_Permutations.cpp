...
ll mod_nCr(ll n, ll r){
	...
	ll ans = mod_div(factorial[n], factorial[r] * factorial[n-r]);
	return ans;
}
...
void solve(){
	...
	if(x == n){
		cout << mod_nCr(x-y-1, j-i-1) * mod_nCr(y-1, n-j) % MOD; return;
	}

	ll ans = 0;
	for(int k = 2; k < j; k++)
		if(k < i)
			(ans += mod_nCr(n-x-1, i-k-1) * mod_nCr(x-y-1, j-i-1)%MOD * mod_nCr(y-1, n-j) % MOD) %= MOD;
		else
			(ans += mod_nCr(n-x-1, k-i-1) * mod_nCr(x-y-1, (j-k-1) - ((n-x-1) - (k-i-1)))%MOD * mod_nCr(y-1, n-j) % MOD) %= MOD;
